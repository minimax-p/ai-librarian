...
class CitationQueryEngine(BaseQueryEngine):
    ...
    def synthesize(
        ...
        response_object = self._response_synthesizer.synthesize(
            query=query_bundle,
            nodes=nodes,
    ...
    def _query(self, query_bundle: QueryBundle) -> RESPONSE_TYPE:
        ...
        with self.callback_manager.event(
            ...
            unique_sources = set()
            full_sources_metadata = ""
            for node in nodes:
                citation_name = node.node.get_content().split('$#$@', 1)[0]
                print(citation_name)
                if citation_name and citation_name not in unique_sources:
                    unique_sources.add(citation_name)
                    citation_num = citation_name.split(' ')[1]

                    citation_text = f"""
                    {citation_name}: 
                    Book / Document Name: {node.node.metadata.get('file_name')}
                    Page: {node.node.metadata.get('page_label')}
                    """
                    full_sources_metadata += f"\n{citation_text}"
            response.response = f"{response.response}\n\n--- CITATIONS ---{full_sources_metadata}"
        return response
